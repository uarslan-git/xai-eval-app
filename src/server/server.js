...
function init() {
  ...
  app.use(
    express.static(
      path.join(__dirname, project_root, client_ui_web_pages_location)
    )
  );
  db_init();
}
...
function start_http_server(port) {
  app.listen(port, () => {
    console.log(
      `Server running at http://localhost:${port}?participant_id=123&study_id=456`
    );
  });
}
  ...
  db = new sqlite3.Database(
    ...
    (err) => {
      if (err) {
        ...
        console.log(
          "Database opened (or created if not existing) successfully."
        );
      }
    }
  );
...
function cb_event_write_db(req, res) {
  ...
  console.log(
    `Checking database for existing entry: participant_id=${participant_id}, study_id=${study_id}, page_nr=${page_nr}`
  );
...
function get_last_updated_page_nr(participant_id, study_id) {
  console.log(
    `Fetching last updated page_nr for: participant_id=${participant_id}, study_id=${study_id}`
  );
...
function get_study_details(study_id) {
  ...
  return new Promise((resolve, reject) => {
    db.get(query, [study_id], (err, row) => {
      ...
      resolve({
        study_type: parseInt(row.study_type),
...
async function cb_event_db_validation_participant_id_and_study_id(req, res) {
  ...
  console.log(
    `[ CLIENT REQUEST ] Submit Participant ID: ${encodeURIComponent(
      participant_id
    )}`
  );
  console.log(
    `[ CLIENT REQUEST ] Submit Study ID: ${encodeURIComponent(study_id)}`
  );

  try {
    const { study_type, total_pages } = await get_study_details(
      parseInt(study_id)
    );
    ...
    if (study_type === -1) {
      return res
        .status(500)
        .json({ error: "DB Error or study table does not exist" });
    } else if (study_type === -2) {
      return res
        .status(404)
        .json({
          error: "Record with study_id does not exist in the study table",
        });
    }

    const last_updated_page_nr = await get_last_updated_page_nr(
      participant_id,
      study_id
    );
...
function cb_event_get_evidence(req, res) {
  ...
  console.log(
    `[EVIDENCE API] Request received - diagnosis: ${diagnosis}, patient: ${patientId}`
  );
  ...
  switch (diagnosis) {
    case "healthy":
      mockEvidence = {
        evidenceFor: [
          {
            concept: "Normal Bone Density",
            importance: 0.88,
            description: "Bone density within healthy range for age",
          },
          {
            concept: "Proper Joint Alignment",
            importance: 0.85,
            description: "All joints show correct alignment and spacing",
          },
          {
            concept: "Strong Cortical Structure",
            importance: 0.82,
            description: "Cortical bone shows excellent structural integrity",
          },
          {
            concept: "No Degeneration Signs",
            importance: 0.79,
            description: "No visible signs of bone degeneration detected",
          },
        ],
        evidenceAgainst: [
          {
            concept: "Minor Imaging Artifacts",
            importance: 0.25,
            description: "Some minor artifacts present in imaging",
          },
          {
            concept: "Natural Variation",
            importance: 0.18,
            description: "Slight natural anatomical variations",
          },
        ],
      };
      break;
    ...
    case "mild":
      mockEvidence = {
        evidenceFor: [
          {
            concept: "Early Bone Changes",
            importance: 0.65,
            description: "Very early signs of bone structure changes",
          },
          {
            concept: "Minor Density Reduction",
            importance: 0.58,
            description: "Slight reduction in bone density detected",
          },
          {
            concept: "Subtle Joint Narrowing",
            importance: 0.52,
            description: "Minimal joint space narrowing observed",
          },
        ],
        evidenceAgainst: [
          {
            concept: "Mostly Normal Structure",
            importance: 0.72,
            description: "Most bone structure remains normal",
          },
          {
            concept: "Good Joint Function",
            importance: 0.68,
            description: "Joint functionality appears preserved",
          },
          {
            concept: "Adequate Cortical Thickness",
            importance: 0.61,
            description: "Cortical bone thickness still adequate",
          },
        ],
      };
      break;
    ...
    case "moderate":
      mockEvidence = {
        evidenceFor: [
          {
            concept: "Noticeable Bone Loss",
            importance: 0.78,
            description: "Clear evidence of bone density loss",
          },
          {
            concept: "Joint Space Reduction",
            importance: 0.75,
            description: "Moderate reduction in joint spacing",
          },
          {
            concept: "Cortical Thinning",
            importance: 0.71,
            description: "Visible thinning of cortical bone",
          },
          {
            concept: "Structural Weakening",
            importance: 0.68,
            description: "Signs of structural weakening present",
          },
        ],
        evidenceAgainst: [
          {
            ...
            importance: 0.52,
            description: "Some areas show preserved structure",
          },
          {
            concept: "Treatable Stage",
            importance: 0.48,
            description: "Condition appears treatable at this stage",
          },
        ],
      };
      break;
    ...
    case "severe":
      mockEvidence = {
        evidenceFor: [
          {
            concept: "Significant Degeneration",
            importance: 0.89,
            description: "Severe bone degeneration clearly visible",
          },
          {
            concept: "Major Joint Damage",
            importance: 0.86,
            description: "Substantial joint space loss and damage",
          },
          {
            concept: "Advanced Thinning",
            importance: 0.83,
            description: "Advanced cortical bone thinning",
          },
          {
            concept: "Structural Compromise",
            importance: 0.81,
            description: "Major structural compromise evident",
          },
          {
            concept: "Multiple Biomarkers",
            importance: 0.78,
            description: "Multiple OCDegen biomarkers present",
          },
        ],
        evidenceAgainst: [
          {
            concept: "Small Healthy Regions",
            importance: 0.32,
            description: "Very small regions show healthier tissue",
          },
          {
            concept: "Treatment Possibility",
            importance: 0.28,
            description: "Aggressive treatment may still help",
          },
        ],
      };
      break;
    ...
    default:
      mockEvidence = {
        evidenceFor: [
          {
            concept: "Critical Bone Degeneration",
            importance: 0.95,
            description: "Severe and extensive bone deterioration",
          },
          {
            concept: "Complete Joint Collapse",
            importance: 0.92,
            description: "Near-complete loss of joint structure",
          },
          {
            concept: "Extreme Cortical Loss",
            importance: 0.91,
            description: "Extreme thinning or loss of cortical bone",
          },
          {
            concept: "Advanced Disease Stage",
            importance: 0.89,
            description: "All markers indicate advanced disease",
          },
          {
            concept: "Systemic Involvement",
            importance: 0.87,
            description: "Multiple bone systems affected",
          },
          {
            concept: "Irreversible Damage",
            importance: 0.84,
            description: "Evidence suggests irreversible damage",
          },
        ],
        evidenceAgainst: [
          {
            concept: "Minimal Preservation",
            importance: 0.18,
            description: "Extremely limited healthy tissue remains",
          },
          {
            concept: "Late Detection",
            importance: 0.15,
            description: "Disease detected at very late stage",
          },
        ],
      };
      break;
  ...
  console.log(
    `[EVIDENCE API] Sending response for ${diagnosis}:`,
    mockEvidence
  );
...
function cb_event_get_heatmap(req, res) {
  ...
  console.log(
    `Fetching heatmap - concept: ${concept}, patient: ${patientId}, diagnosis: ${diagnosis}`
  );
  ...
  const conceptSlug = concept
    ...
    : "default";
  const heatmapPath = `/visualizations/patient_${
    patientId || "default"
  }/heatmap_${conceptSlug}.png`;
...
function cb_event_get_patient_visualizations(req, res) {
  ...
  console.log(
    `[VISUALIZATIONS API] Fetching visualizations for patient: ${patientId}`
  );
  ...
  const availablePatients = {
    patient_35: {
      heatmaps: [
        {
          id: 1,
          name: "Feature Importance Heatmap",
          url: "/visualizations/patient_35/heatmap_1.png",
        },
        {
          id: 2,
          name: "Regional Analysis Heatmap",
          url: "/visualizations/patient_35/heatmap_2.png",
        },
        {
          id: 3,
          name: "Bone Structure Heatmap",
          url: "/visualizations/patient_35/heatmap_3.png",
        },
        {
          id: 4,
          name: "Overall Confidence Heatmap",
          url: "/visualizations/patient_35/heatmap_4.png",
        },
      ],
      waterfall: {
        url: "/visualizations/patient_35/waterfall.png",
        name: "SHAP Waterfall Plot",
      },
    },
    patient_9: {
      heatmaps: [
        {
          id: 1,
          name: "Feature Importance Heatmap",
          url: "/visualizations/patient_9/heatmap_1.png",
        },
        {
          id: 2,
          name: "Regional Analysis Heatmap",
          url: "/visualizations/patient_9/heatmap_2.png",
        },
        {
          id: 3,
          name: "Bone Structure Heatmap",
          url: "/visualizations/patient_9/heatmap_3.png",
        },
        {
          id: 4,
          name: "Overall Confidence Heatmap",
          url: "/visualizations/patient_9/heatmap_4.png",
        },
      ],
      waterfall: {
        url: "/visualizations/patient_9/waterfall.png",
        name: "SHAP Waterfall Plot",
      },
    },
    patient_3: {
  ...
  const defaultResponse = {
    heatmaps: [
      {
        id: 1,
        name: "Sample Feature Heatmap",
        url: "/visualizations/sample_heatmap_1.png",
        isMock: true,
      },
      {
        id: 2,
        name: "Sample Analysis Heatmap",
        url: "/visualizations/sample_heatmap_2.png",
        isMock: true,
      },
    ],
    waterfall: {
      url: "/visualizations/sample_waterfall.png",
      name: "Sample Waterfall Plot",
      isMock: true,
    },
  };
  ...
  console.log(
    `[VISUALIZATIONS API] Sending visualizations for ${patientId}:`,
    visualizations
  );
...
init();
register_post_event(
  "/db_validation_participant_id_and_study_id",
  cb_event_db_validation_participant_id_and_study_id
);
...
register_get_event(
  "/api/visualizations/:patientId",
  cb_event_get_patient_visualizations
);
